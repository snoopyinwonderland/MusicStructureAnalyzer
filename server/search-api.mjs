...
const translateInstrumentNamesInXml=xml=>xml.replace(/<(part-name|part-abbreviation)(\b[^>]*)>([^<]+)<\/\1>/g,(whole,tag,attrs,name)=>`<${tag}${attrs}>${translateInstrumentName(name)}</${tag}>`);
...
export function searchDatabase(query,limit=100){
 const profileEnabled=process.env.MUSICANOTE_PROFILE_SEARCH==='1',profileStart=performance.now(),profile={retrievalMs:0,decodeMs:0,exactScanMs:0,alignmentMs:0,occurrenceScanMs:0,windowPrepMs:0,scoreMs:0,meterLookupMs:0,meterPersistMs:0,metricScoringMs:0,candidateEvaluationMs:0,enrichmentMs:0},profiled=(name,fn)=>{if(!profileEnabled)return fn();const started=performance.now();try{return fn()}finally{profile[name]+=performance.now()-started}};
 ...
 const allowed=query.scopeWorkIds?.length?new Set(query.scopeWorkIds):null,candidateEntries=[...hits].filter(([key])=>!allowed||allowed.has(key.split('\u0000')[0])).map(([key,evidence])=>[key,evidence+intervalRetrievalBonus(intervalHits.get(key),intervalGramCount)]),candidateLimit=query.absoluteExactOnly?1800:notes.length>=8?1400:350,startsPerWork=notes.length>=8?8:3,seedCandidates=diversifyCandidates(candidateEntries,candidateLimit,startsPerWork),workEvidence=new Map();
 ...
 for(const [key,hitCount] of candidates){
  ...
  let prepared=requestWorks.get(id);if(!prepared){prepared=profiled('decodeMs',()=>{const row=get.get(id);if(!row)return null;const all=JSON.parse(row.notes);return{row,all,allNotes:corpusNotes(id,all)}});if(!prepared)continue;requestWorks.set(id,prepared)}
  const {row,all,allNotes}=prepared;let spans=spanCache.get(id),spanStarted=profileEnabled&&!spans?performance.now():0;if(!spans){
...
export function searchCatalog(text,limit=12){
 ...
 if(hasFts){const terms=queryTokens.map(term=>`"${term.replaceAll('"','""')}"*`).join(' AND ');try{rows=store.prepare('SELECT w.id,w.title,w.composer,w.source,w.stream_id,w.role,bm25(works_fts,0,5,4,3,2,1) rank FROM works_fts JOIN works w ON w.id=works_fts.id WHERE works_fts MATCH ? ORDER BY rank,w.role DESC LIMIT ?').all(terms,Math.max(limit*100,1000))}catch{rows=[]}}
 if(rows.length){const titleRows=rows.filter(row=>catalogTitleMatches(row.title,query));if(titleRows.length)rows=titleRows}
 if(!rows.length){const escaped=queryTokens.map(term=>`%${term.replace(/[\\%_]/g,value=>`\\${value}`)}%`),clause=queryTokens.map(()=>"(lower(title) LIKE ? ESCAPE '\\' OR lower(normalized_title) LIKE ? ESCAPE '\\')").join(' AND '),params=escaped.flatMap(pattern=>[pattern,pattern]);rows=store.prepare(`SELECT id,title,composer,source,stream_id,role,0 rank FROM works WHERE ${clause} ORDER BY role DESC,title LIMIT ?`).all(...params,Math.max(limit*20,240)).filter(row=>catalogTitleMatches(row.title,query))}
 if(!rows.length){const escaped=queryTokens.map(term=>`%${term.replace(/[\\%_]/g,value=>`\\${value}`)}%`),haystack="lower(coalesce(title,'')||' '||coalesce(normalized_title,'')||' '||coalesce(composer,'')||' '||coalesce(source,''))",clause=queryTokens.map(()=>`${haystack} LIKE ? ESCAPE '\\'`).join(' AND ');rows=store.prepare(`SELECT id,title,composer,source,stream_id,role,0 rank FROM works WHERE ${clause} ORDER BY role DESC,title LIMIT ?`).all(...escaped,Math.max(limit*8,80))}
 const seen=new Set(),items=[];for(const row of rows){const title=repairMetadataText(row.title),key=normalizeYoutubeTitle(title);if(seen.has(key))continue;seen.add(key);items.push({workId:row.id,title,composer:displayComposer(repairMetadataText(row.composer||'')),source:row.source,partName:sourcePartName(row.source,row.stream_id)});if(items.length>=limit)break}return items;
}
