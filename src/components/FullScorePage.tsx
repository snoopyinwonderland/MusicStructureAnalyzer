...
function MotifReviewPanel({span}:{span:MotifSpanMarker}){
  const measures=Number(span.startNote.measure)===Number(span.endNote.measure)?`${span.startNote.measure}마디`:`${span.startNote.measure}–${span.endNote.measure}마디`,start=span.startNote.spelling||pitchName(span.startNote.pitchMidi),end=span.endNote.spelling||pitchName(span.endNote.pitchMidi);
...
export function FullScorePage() {
  ...
  const motifSpans=useMemo(()=>work&&!restricted?buildMotifSpans(work.notes,work.phraseAnalysis?.motifRelations||[],buildPhraseSpans(work.notes,supportedPhraseBoundaries)):[],[work,restricted,supportedPhraseBoundaries]);
