...
type PhraseAnalysis = { version: string; calibrated: false; label: string; boundaries: PhraseBoundary[]; motifRelations?:MotifRelation[] };
...
export function buildMotifSpans(notes:CorpusNote[],relations:MotifRelation[]=[]):MotifSpanMarker[]{
  const seen=new Set<string>(),spans:Array<Omit<MotifSpanMarker,'motifNumber'>>=[];
  ...
  for(const relation of relations){
    if(!relation.signatureKey||!relation.closeContinuation)continue;
    ...
    for(const [startIndex,exclusiveOnset] of [[relation.previousIndex,currentOnset],[relation.currentIndex,currentOnset+cycleDuration]] as const){
      const endIndex=cycleEnd(startIndex,exclusiveOnset),key=`${startIndex}:${endIndex}`;
      if(endIndex===null||seen.has(key))continue;seen.add(key);
      spans.push({startIndex,endIndex,startNote:notes[startIndex],endNote:notes[endIndex]});
    }
  }
  return spans.sort((a,b)=>a.startIndex-b.startIndex||a.endIndex-b.endIndex).map((span,index)=>({...span,motifNumber:index+1}));
}
...
export function FullScorePage() {
  ...
  const motifSpans=useMemo(()=>work&&!restricted?buildMotifSpans(work.notes,work.phraseAnalysis?.motifRelations||[]):[],[work,restricted]);
  ...
  return <main className="score-page">
    ...
    {work && <div className="score-layout">
      <div className="score-main">
        ...
        <FullXmlNotation xml={work.xml} start={start} end={end} targets={matches} playbackNotes={playbackNotes} activePlaybackIndex={activePlaybackIndex} onPlaybackRangeChange={updateRenderedPlaybackRange} streamId={work.streamId} autoScroll={!browse} measureOrdinalOffset={ordinalOffset} phraseMarkers={phraseMarkers} phraseSpans={phraseSpans} motifSpans={motifSpans} phraseVisible={phraseEnabled} motifVisible={phraseEnabled} selectedPhraseBoundary={selectedPhraseBoundary} onPhraseBoundarySelect={selectPhraseBoundary} flashingPhraseNumber={flashingPhraseNumber} phraseFlashToken={phraseFlashToken} onPhraseSelect={selectPhrase} />
      </div>
      <aside>
